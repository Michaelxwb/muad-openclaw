...
func (s *Store) migrateLLMModelThinking() error {
	exists, err := columnExists(s.db, "llm_model_configs", "thinking")
	if err != nil {
		return fmt.Errorf("inspect LLM model thinking column: %w", err)
	}
	if exists {
		return nil
	}
	if _, err := s.db.Exec(`ALTER TABLE llm_model_configs
		ADD COLUMN thinking TEXT NOT NULL DEFAULT 'off'
		CHECK (thinking IN ('off','minimal','low','medium','high','xhigh','max'))`); err != nil {
		return fmt.Errorf("add LLM model thinking column: %w", err)
	}
	return nil
}
