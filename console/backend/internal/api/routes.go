...
func (s *Server) registerExistingSettingsRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("PATCH /api/v1/llm/models/{modelConfigId}", s.handleUpdateLLMModel)
	...
	mux.HandleFunc("GET /api/v1/skills/{skillId}/download", s.handleDownloadSkill)
	...
	mux.HandleFunc("POST /api/v1/skills/{skillId}/approve", s.handleApproveSkill)
	mux.HandleFunc("POST /api/v1/skills/{skillId}/reject", s.handleRejectSkill)
	...
	mux.HandleFunc("GET /api/v1/long-tasks", s.handleListLongTasks)
