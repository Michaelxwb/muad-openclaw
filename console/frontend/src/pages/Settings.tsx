...
export function Settings() {
  ...
  useEffect(() => {
    api
      .getResources()
      .then((c) =>
        setForm({ memLimit: c.memLimit, cpuLimit: c.cpuLimit, restartPolicy: c.restartPolicy }),
      )
  ...
  return (
    <div>
      ...
      <Card title="容器资源默认配置（全局）">
        <p className="hint">
          新建/重建容器时套用。已运行容器改动后需<b>重建</b>才生效。每用户可在容器页单独覆盖。
        </p>
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          <Select
            ...
            onChange={(v) => field("restartPolicy", v as string)}
            style={{ width: 300 }}
          />
        </div>
        <p className="hint">
          重启策略 = 容器异常退出或宿主重启后是否自动拉起：
          <br />· <b>除手动停止外总重启</b>：崩溃/宿主重启都拉起，但手动停的保持停止（默认推荐）
          <br />· <b>总是重启</b>：连手动停的也会被拉起
