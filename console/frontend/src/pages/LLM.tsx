  ...
  return (
    <div>
      ...
      <Card title={`全局配置 ${configured ? "" : "（未配置）"}`} style={{ marginBottom: 16 }}>
        ...
        >
          <label style={{ fontSize: 12, fontWeight: 600, color: "var(--semi-color-text-2)" }}>
            provider
          </label>
          {input("provider")}
          <label style={{ fontSize: 12, fontWeight: 600, color: "var(--semi-color-text-2)" }}>
        ...
        {(testOutput || testError) && (
          <pre
            style={{
              ...
              fontSize: 12,
              color: testError ? "var(--semi-color-danger)" : "var(--semi-color-success)",
            }}
          >
            {testOutput || testError}
          </pre>
      ...
      <Card title="批量应用与覆盖" style={{ marginBottom: 16 }}>
        <p className="hint">
          改全局后只影响新建；勾选容器「应用到所选」会<b>完整重建容器</b>
          （保留状态卷），不是热加载。
          {!tested && (
        ...
        <Button onClick={apply} disabled={!tested}>
          应用到所选
        </Button>

        <div
          style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--semi-color-border)" }}
        >
          <h4 style={{ fontSize: 14, color: "var(--semi-color-text-2)", margin: "0 0 10px" }}>
            单用户覆盖
          </h4>
          <p className="hint">
            保存覆盖时会先做连通性测试，不通过则不保存；保存后需对该用户「应用到所选」才生效。
          </p>
          <div
            style={{
              display: "grid",
