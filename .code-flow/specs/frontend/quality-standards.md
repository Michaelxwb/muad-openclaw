...
- 会话 token 仅经 `token` helper 存 `localStorage` 的约定 key；除管理员人用户平台凭证页面按产品决策明文查看/覆盖外，禁止另存平台认证 credential、binding code 明文
...
- 禁止在 UI 展示原始堆栈或内部路径
- 除管理员人用户平台凭证页面外，禁止把平台认证 credential / binding code 渲染进 DOM；LLM 模型 API Key 按产品决策允许在模型管理页明文展示
