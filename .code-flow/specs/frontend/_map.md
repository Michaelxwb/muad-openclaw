...
muad 管理控制台前端 SPA。管理员通过浏览器登录后管理 Pod、Human User、IM Identity、业务平台凭证、Skill 资产、模型池配置、资源平台和审计日志。
...
- **Styling**: CSS Modules（组件级样式隔离，`*.module.css`）+ 全局设计 tokens（`:root` 变量，`styles.css`）+ Semi Design 组件
...
| `src/components/AppShell.tsx` | 已登录控制台壳：一级导航、用户信息、主题、通知与页面切换 |
...
| `src/components/ConsolePage.tsx` | PageHeader/PageSection/ListToolbar/MetricDescriptions/FeedbackBanner 等 Semi 页面骨架 |
...
| `src/components/Pagination.tsx` | Semi Table 分页封装：默认 10 条，页量 10/20/50/100，页量选择在底部分页条左侧 |
| `src/components/human-users/HumanUsersPanel.tsx` | Pod 内 Human User 管理入口：列表刷新、创建、详情与一次性激活码对话框编排 |
| `src/components/human-users/HumanUserDetailDialog.tsx` | Human User 详情容器，组合基本信息、Identity/绑定码、平台凭证、Skill 和删除操作 |
| `src/components/human-users/HumanUserSkillsTab.tsx` | 用户维度最终生效 Skill 视图，支持 private skill 上传、搜索和状态过滤 |
...
| `src/components/Select.tsx` / `Modal.tsx` | 旧自定义控件仍存在，新增页面优先用 Semi Select/Modal/Table/Card |
...
| `src/pages/Users.tsx` | 全局 Human User 列表视图，支持跳转 Pod、搜索过滤和创建用户 |
| `src/pages/Skills.tsx` / `pages/skills/PublicSkillUploadDialog.tsx` | Skill 管理页面：Public Skill 上传、扫描、应用、列表、详情和状态操作 |
| `src/pages/LLM.tsx` / `pages/llm/LLMCreateDialog.tsx` | 模型池配置页面：批量创建、连通性测试、绑定状态和 key fingerprint 展示 |
| `src/pages/Settings.tsx` | 资源默认值与业务平台配置页面 |
| `src/pages/Audit.tsx` | 审计日志页面：ListToolbar + Semi Table + 统一分页 |
...
│   │   ├── human-users/      # Human User 创建、列表、详情、平台凭证和 Skill
│   │   ├── platforms/        # 业务平台配置
│   │   ├── Sidebar.tsx
│   │   ├── Sidebar.module.css
...
Pagination (server-side lists):
  ...
  listSkills/listHumanUsers/listLLMModels → backend pagination
  → Semi Table pagination via tablePagination/renderTablePagination at table bottom
```
...
- 共享组件 → 新增列表页优先复用 `ConsolePage.ListToolbar/PageSection/FeedbackBanner` + `Pagination.tablePagination/renderTablePagination` + Semi Table；操作放左侧，搜索/过滤/刷新放右侧
