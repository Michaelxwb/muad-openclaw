...
1. **AGENTS.md 指令**：AI 根据问题自动判断领域，读取 `_map.md` 导航地图理解项目结构
2. **code-flow 插件**：在你发送消息时根据引用的文件自动注入匹配的约束 specs

两层职责分离：AGENTS.md 负责导航（Tier 0），插件负责约束注入（Tier 1）。
...
6. 更新 `.code-flow/.inject-state`，将该域所有 spec 路径标记为已注入，防止插件重复注入

> 注：手动 inject 会加载该域的**全部** spec，不做标签过滤。这是与插件自动注入的区别——插件按标签精准匹配，手动注入用于全量预览或强制刷新。
