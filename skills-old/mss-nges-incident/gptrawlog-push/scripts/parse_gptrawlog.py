...
def resolve_archive_root(output_arg=None):
    ...
    返回的根总是可写的统一事件归档根，各处理技能（gptrawlog-push /
    mss-event-autoflow / monitor-security-events 等）共用同一根，不做按
    skill / 按 CGID / 按临时目录拆分。
    """
    if output_arg:
...
def collect_entities(gpt_input):
    ...
    - ti_entities: threatList 中命中情报的实体 —— 调用 ioc-context 补充情报的对象
...
def main():
    ...
    md.append("> 以下实体命中威胁情报，后续调用 ioc-context 补充研判依据与遏制范围\n")
    if ti_entities:
