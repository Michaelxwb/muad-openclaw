...
def _call_create_api(cookie: str, batch_dev_infos: list) -> list:
    ...
    try:
        response = requests.post(POLICY_CHECK_CREATE_URL, headers=headers, json=payload, timeout=30, verify=False)
        if response is None:
        ...
        try:
            ...
            log(f"json failed, response: {response}")
            raise e
        ...
        raise RuntimeError(f"下发策略检查任务请求异常: {e}")


# 全局变量，供 _call_create_api 内部使用
