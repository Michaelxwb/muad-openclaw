...
AGENTS.md 的完整内容由适配器模板统一定义，`code-flow init` 已部署到项目根；cf-init 不再内嵌副本，避免双份维护导致不一致。

- 读取现有 AGENTS.md，根据检测到的技术栈填充 `## Team Identity` 占位符（team / project / language）。
- 若用户已自定义，只补充缺失的 `##` 段落，不覆盖已有内容；写入前展示 diff 供确认。
- 若 AGENTS.md 缺失，提示用户先运行 `code-flow init --platform=codex`，不要手工拼装完整模板。

### 6. 生成 .codex/hooks.json 和 .codex/config.toml

检查 `.codex/hooks.json` 是否存在。如果不存在，用 apply_patch 创建，结构必须是三层：event → [{ hooks: [{ type, command }] }]，并包含 `SessionStart` 与 `UserPromptSubmit`。
