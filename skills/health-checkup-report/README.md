...
当前版本以 MSSW（客户数据）+ MSSP/EASM（暴露面/漏洞/弱口令/防护表）作为业务主链路生成 HTML 报告。登录态由 muad（session-manager）代管，脚本经 `src/session.js` 自动注入两平台 Cookie 与平台域名；输出统一写入 `SKILL_OUTPUT_DIR`（非 muad 环境回退到系统临时目录）。旧 XDR 链路已整体移除。

## Run（muad 环境）

登录态、域、输出目录均由运行时自动注入，仅需业务参数：
...
- 都不传时，脚本会通过 MSSW 项目列表接口自动推导（30 天上限内截取）
- 默认开始时间取所有 `billing_start_time` 的最小值
- 默认结束时间取 `min(报告生成时刻, 所有非空 billing_end_time 的最小值)`
...
输出默认写入 `SKILL_OUTPUT_DIR`；非 muad 调试时可用 `--output-dir` 指定。结构化数据会落盘到 `output/report-data.json`，可用 `--output-json` 指定路径。

默认只输出简洁摘要；如需完整 JSON 结果，加 `--json`。

生成流程会在所有数据统计和 Word 导出完成后、ZIP 打包前，使用 `excel-beautifier` 的 `classic` 主题统一美化 `report/风险清单` 内的 Excel。默认启用；传入 `--beautify-excel false` 可跳过。单个表美化失败时会保留原文件并继续生成报告。

**产物命名要求**：所有对外交付物严格中文命名——ZIP 固定为 `安全体检报告.zip`；ZIP 内部风险清单目录为 `风险清单/`（内含 `资产清单.xlsx`、`暴露面清单.xlsx`、`弱口令清单.xlsx`、`漏洞清单.xlsx`、`安全事件清单.xlsx`、`策略检查清单.xlsx`）；HTML/Word 用 `【深信服】安全体检报告-<客户>-<时间>.*`。禁止 `report.zip`、`risk_list` 等英文名。

说明：

- mssw / mssp Cookie 均由 `src/session.js` 自动注入，无需也不得手动配置 Cookie 文件路径
- 域名与接口端点统一由 `config/api_config.json` 集中管理（当前默认：mssw=`http://mssw-inner.sangfor.com.cn:30001`、mssp=`http://soar-inner.sangfor.com.cn:30001`）；可用 `HEALTH_CHECKUP_MSSW_BASE_URL` / `HEALTH_CHECKUP_MSSP_BASE_URL` 覆盖。所有请求统一带 `Host: inner.sangfor.com.cn` 请求头
