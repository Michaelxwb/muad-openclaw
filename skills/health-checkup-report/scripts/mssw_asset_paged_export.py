...
用法：
    python mssw_asset_paged_export.py \
        ...
        --base-url http://mssw-inner.sangfor.com.cn:30001 \
        --company-id 88602882 \
...
# 集中 api_config：取统一 Host 头；base_url 走 --base-url（Node 侧注入完整 host[:port]）
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'shared'))
from api_config_loader import get_host_header, get_origin      # noqa: E402

HOST_HEADER = get_host_header()


def resolve_origin(base_url):
    """把 host 或 host:port 归一化为带 http:// 的完整 origin（去尾部斜杠）。"""
...
def fetch_total(base_url, headers, search_type):
    url = f'{resolve_origin(base_url)}/apps/asset/view/asset/asset_view/count?_method=GET'
...
def fetch_ids_page(base_url, headers, search_type, start, limit):
    url = f'{resolve_origin(base_url)}/apps/asset/view/asset/asset_view/list?_method=GET'
...
def export_batch(base_url, headers, search_type, ids_chunk, export_fields):
    url = f'{resolve_origin(base_url)}/apps/asset/view/asset/export'
    body = {
        'branch_id': 'all',
...
def download_batch(base_url, headers, filename, dest_path):
    url = f'{resolve_origin(base_url)}/apps/asset/view/asset/download_file?file={urllib.parse.quote(filename)}'
...
def parse_args():
    ...
    parser.add_argument('--base-url', default=get_origin('mssw'))
