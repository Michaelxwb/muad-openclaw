...
# 集中 api_config：默认 base URL 走 config（env 可覆盖），请求统一带 Host 头
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'shared'))
from api_config_loader import get_origin as _get_origin, get_host_header as _get_host_header  # noqa: E402


# ==================== 配置（由 export_prevention_table.py 注入覆盖；此处为直接运行兜底） ====================

# --- 通用（默认相对路径，避免写死 Windows 盘符；正式运行由 SKILL_OUTPUT_DIR 注入覆盖） ---
TEMP_DIR     = os.path.join(os.getcwd(), 'tmp_report')
OUTPUT_FILE  = os.path.join(os.getcwd(), '弱口令清单.xlsx')
...
# --- EASM 平台（origin 默认线上，env 可覆盖；正式运行由 export_prevention_table.py 注入覆盖） ---
SOAR_BASE_URL    = _get_origin("mssp")
EASM_COOKIES_FILE = os.path.join(os.getcwd(), 'easm_cookies.txt')

# --- MSSW 平台（origin 默认线上，env 可覆盖） ---
...
MSSP_CSRF_TOKEN = "6c5c52cd-3ebb-47e2-946c-15c2569836e7"
# ==========================================================
...
def request_with_retry(method: str, url: str, base_url: str,
    ...
    if str(base_url).rstrip("/") == str(SOAR_BASE_URL).rstrip("/"):
        headers["X-CSRFToken"] = MSSP_CSRF_TOKEN
