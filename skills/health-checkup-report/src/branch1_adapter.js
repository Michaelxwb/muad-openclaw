...
function getBranch1Root() {
  return path.join(path.resolve(__dirname, '..'), 'branch1');
}

function getDefaultDeviceJsonPath() {
  return require('./workdir').deviceJsonPath();
}

async function runBranch1ReportStage(options = {}) {
  const branch1Root = getBranch1Root();
  const reportDir = path.join(branch1Root, 'report');
  const outputDir = path.resolve(options.outputDir || require('./workdir').branch1WorkDir());
  await fs.mkdir(outputDir, { recursive: true });

  const reportOutputPath = path.join(outputDir, 'branch1-report.json');
  const policyJsonPath = options.policyJsonPath || require('./workdir').policyJsonPath();
  const policyExcelPath = options.policyExcelPath || require('./workdir').policyExcelPath();
  const args = [
...
async function findChecklistArtifacts(options = {}) {
  const candidates = [];
  const extractedDir = path.join(require('./workdir').preventionWorkDir(), 'extracted');
...
function execPython(args, label, options = {}, logger) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(require('./python').pythonCommand(), args, {
      cwd: options.cwd,
      env,
      windowsHide: true
    });
