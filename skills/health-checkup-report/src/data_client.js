...
async function collectReportData(input) {
  if (input.msswCookiePath) {
    ...
    // 资产台账接口与攻击态势接口相互独立，保守并发 2 并行请求以缩短整体耗时。
    // 攻击态势失败不阻断主流程（fetchAttackOverview 内部已兜底返回 null）。
    const [overviewData, attackOverviewData] = await Promise.all([
    ...
    merged.attackOverview = attackOverviewData || buildEmptyAttackOverview();
    return merged;
  }
...
/**
 * 调用安全体检报告统计接口，计算攻击态势总览数据。
 * 失败返回 null 且不阻断主流程，仅在日志中提示。
 */
async function fetchAttackOverview(input) {
