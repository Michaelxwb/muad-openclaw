...
// origin 统一由 config/api_config.json 管理（线上默认，env 可覆盖）；含协议与端口
const DEFAULT_MSSW_BASE_URL = apiConfig.getOrigin('mssw');
const DEFAULT_SOAR_BASE_URL = apiConfig.getOrigin('mssp');
...
/** 返回完整 origin（协议+host+端口，去尾部斜杠）。输入可为完整 URL 或裸 host。 */
...
// readXdrCookieInfo 已移除（XDR 链路去除，凭据统一走 session-manager）
...
async function fetchIncidentAttckCount(cookieInfo, msswBaseUrl, companyId, alertIds) {
  ...
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${ATTCK_COUNT_ENDPOINT}`;
...
async function fetchIncidentAlertIds(cookieInfo, msswBaseUrl, companyId, incidentId) {
  const headers = buildMsswTableQueryHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${DISPOSAL_TABS_ENDPOINT}/${incidentId}`;
...
async function queryCaseStudyAlertRow(msswCookieInfo, msswBaseUrl, companyId, range, alertIds, stageId, techniqueId) {
  ...
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${ALERT_QUERY_ENDPOINT}`;
...
async function queryCaseStudyIncidentTimeline(cookieInfo, msswBaseUrl, companyId, range, incidentId) {
  const headers = buildMsswTableQueryHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${INCIDENT_TABLE_QUERY_ENDPOINT}`;
  const response = await requestJson(url, {
    headers,
...
async function fetchMsswAlertTableCount(cookieInfo, msswBaseUrl, companyId, options) {
  ...
  const msswHost = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const url = msswHost + ALERT_QUERY_ENDPOINT;
...
function readIncidentCompletionTime(excelPath, incidentId) {
  return new Promise((resolve, reject) => {
    ...
    execFile(require('./python').pythonCommand(), [scriptPath, encodePath(excelPath), incidentId], {
      encoding: 'utf8',
      timeout: 30000,
...
async function processRiskListTable(tableType, inputPath, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    execFile(require('./python').pythonCommand(), [scriptPath, tableType, encodePath(inputPath), encodePath(outputDir), ...extraArgs], { encoding: 'utf8', timeout: 1800000, env: Object.assign({}, process.env, { PYTHONIOENCODING: 'utf-8' }) }, (error, stdout, stderr) => {
      if (error) {
        reject(new Error(`处理风险清单表失败: ${stderr || error.message}`));
        return;
      }
      try {
...
async function fetchMsswAssetCore(cookieInfo, msswBaseUrl, companyId) {
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_ASSET_COUNT_ENDPOINT}`;
  const response = await requestJson(url, {
    headers,
...
async function fetchMsswIncidentTablePage(cookieInfo, msswBaseUrl, companyId, options) {
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_INCIDENT_TABLE_ENDPOINT}`;
  const response = await requestJson(url, {
    headers,
...
// readSoarCookieInfo 已移除（凭据统一走 session-manager 的 mssw/mssp 平台）

function buildMsswHeaders(cookieString, baseUrl, overrides = {}) {
  const msswBaseUrl = resolveBaseUrl(baseUrl || DEFAULT_MSSW_BASE_URL);
  return {
    host: HOST_HEADER,
    accept: 'application/json, text/plain, */*',
    'accept-language': 'zh-CN,zh;q=0.9',
    'content-type': 'application/json',
    cookie: cookieString,
    referer: `${msswBaseUrl}${apiConfig.getRefererPath('mssw')}`,
    'sec-ch-ua': '"Google Chrome";v="125", "Chromium";v="125", "Not.A/Brand";v="24"',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '"Windows"',
    'sec-fetch-dest': 'empty',
...
async function triggerMsswIncidentExport(cookieInfo, msswBaseUrl, options) {
  ...
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_INCIDENT_EXPORT_ENDPOINT}`;
...
async function pollMsswIncidentExportTask(cookieInfo, msswBaseUrl, taskId, options = {}) {
  ...
  const companyId = options.customerId || options.companyId || '';
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_INCIDENT_EXPORT_ENDPOINT}/query`;
  let lastStatus = '';
...
async function downloadMsswIncidentFile(cookieInfo, msswBaseUrl, taskId, downloadDir, filename, companyId) {
  const msswHost = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const downloadUrl = `${msswHost}${MSSW_INCIDENT_EXPORT_ENDPOINT}/${taskId}/download`;
...
async function exportMsswIncidentList(options) {
  ...
  // 保留 origin 协议与端口（:30001），不能用 normalizeBaseUrl（会去端口）
  const msswBaseUrl = resolveBaseUrl(options.msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const companyId = options.customerId || options.companyId || '';
...
async function fetchMsswExportFields(cookieInfo, msswBaseUrl, companyId) {
  ...
  const url = `${msswHost}${MSSW_ASSET_EXPORT_FIELDS_ENDPOINT}`;
...
async function triggerMsswAssetExport(cookieInfo, msswBaseUrl, companyId, exportFields, ids = [], searchType = 'current') {
  const msswHost = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const headers = buildMsswAssetExportHeaders(cookieInfo, msswHost, companyId);
  const url = `${msswHost}${MSSW_ASSET_EXPORT_ENDPOINT}`;
...
async function downloadMsswAssetFile(cookieInfo, msswBaseUrl, companyId, filename, downloadDir) {
  const msswHost = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const downloadUrl = `${msswHost}${MSSW_ASSET_DOWNLOAD_ENDPOINT}?file=${encodeURIComponent(filename)}`;
...
async function fetchMsswCustomerListPage(cookieInfo, msswBaseUrl, { companyId, keyword, offset, limit }) {
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_CUSTOMER_STATISTIC_ENDPOINT}?_method=GET`;
  const response = await requestJson(url, {
    headers,
    body: JSON.stringify({
      order: 'desc',
...
async function fetchMsswProjectListByCompanyId(cookieInfo, msswBaseUrl, companyId) {
  ...
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${MSSW_PROJECT_LIST_ENDPOINT}`;
  const response = await requestJson(url, {
    headers,
    body: JSON.stringify({
...
async function exportMsswDeviceList(options) {
  ...
  const cookieInfo = await readMsswCookieInfo(options.msswCookiePath);
  // 设备列表需保留 origin 的协议与端口（:30001），不能用 normalizeBaseUrl（会去端口）
  const msswBaseUrl = resolveBaseUrl(options.msswBaseUrl || DEFAULT_MSSW_BASE_URL);
  const companyId = options.customerId || options.companyId || '';
  // /api/apex/device/v1/devices/list 不支持分页参数（带分页会 404），实际返回全量设备；因此不再翻页
  const firstResponse = await fetchMsswDeviceList(cookieInfo, msswBaseUrl, companyId, 0, 1, [1, 2, 3, 4]);
  const firstData = firstResponse && firstResponse.data && typeof firstResponse.data === 'object' ? firstResponse.data : {};
  const devices = Array.isArray(firstData.list) ? firstData.list : [];
  ...
  const outputPath = require('./workdir').deviceJsonPath();
...
async function fetchMsswAssetOverview(options = {}) {
  ...
  // 保留 origin 协议与端口（:30001），不能用 normalizeBaseUrl（会去端口）
  const xdrBaseUrl = resolveBaseUrl(options.msswBaseUrl || DEFAULT_MSSW_BASE_URL);
...
async function fetchMsswDeviceList(cookieInfo, msswBaseUrl, companyId, pageSize, pageNum = 1, devStatus) {
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${DEVICE_LIST_ENDPOINT}`;
  const response = await requestJson(url, {
    headers,
    // 此接口不接受分页参数（带 pageSize/pageNum 会返回 404），实际返回全量设备；与策略检查 device_info 请求体一致
    body: JSON.stringify({
      keyword: '',
      type: 0,
...
async function fetchMsswThirdPartyDeviceStats(cookieInfo, msswBaseUrl, companyId) {
  const headers = buildMsswExportHeaders(cookieInfo, msswBaseUrl, companyId);
  const url = `${resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL)}${THIRD_PARTY_DEVICE_STATS_ENDPOINT}`;
    const response = await requestJson(url, {
    headers,
    body: JSON.stringify({
      name: '',
...
async function collectMsswDeviceCategoryCounts(cookieInfo, msswBaseUrl, companyId, logger) {
  ...
  // 接口不支持分页（带分页参数 404），返回全量设备；不再翻页
  const devices = Array.isArray(firstPageData.list) ? firstPageData.list : [];
...
function buildMsswLogSearchCountHeaders(cookieInfo, msswBaseUrl) {
  ...
  return {
    ...
    'content-type': 'application/json',
    cookie: cookieInfo.cookieString,
    referer: `${msswHost}/ui-analyze/index.html`,
    'sec-ch-ua': '"Google Chrome";v="125", "Chromium";v="125", "Not.A/Brand";v="24"',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '"Windows"',
    'sec-fetch-dest': 'empty',
...
async function fetchMsswLogCountByTable(cookieInfo, msswBaseUrl, companyId, tableId, fromDate, toDate) {
  const url = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL) + MSSW_LOG_SEARCH_COUNT_ENDPOINT;
...
async function removeIncidentSensitiveColumns(inputPath, outputDir) {
  ...
  return new Promise((resolve, reject) => {
    execFile(require('./python').pythonCommand(), [scriptPath, encodePath(inputPath), encodePath(resolvedOutputDir)], { encoding: 'utf8', timeout: 60000, env: Object.assign({}, process.env, { PYTHONIOENCODING: 'utf-8' }) }, (error, stdout, stderr) => {
      if (error) {
        reject(new Error(`删除事件表敏感列失败: ${stderr || error.message}`));
        return;
      }
      try {
        resolve(JSON.parse(stdout));
...
async function fetchSecurityCheckReportStats(cookieInfo, msswBaseUrl, customerId, customerName, start, end) {
  ...
  const url = resolveBaseUrl(msswBaseUrl || DEFAULT_MSSW_BASE_URL) + SECURITY_CHECK_REPORT_STATS_ENDPOINT;
  const headers = buildMsswLogSearchCountHeaders(cookieInfo, msswBaseUrl);
  const body = JSON.stringify({
    condition: {
