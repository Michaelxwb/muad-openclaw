...
class HtmlToWordExporter:
    ...
    def render_html(self):
        ...
        # 优先定位系统/预置 Chromium（避免依赖 playwright 自带 chromium 下载）
        ...
        if chrome_path:
            ...
            _log(f"使用 Chromium: {chrome_path}")
        try:
            browser = self._playwright_ctx.chromium.launch(**launch_kwargs)
        except Exception as e:
            if not chrome_path:
                raise
            _log(f"指定 Chromium 启动失败，回退到 Playwright 自带 chromium: {e}", "WARNING")
    ...
    def _find_system_chrome():
        """查找系统 / OpenClaw / Playwright 预置的 Chrome / Chromium 可执行文件路径（跨平台）。

        探测顺序（Windows 与 Linux 均兼容，避免依赖 playwright 自带浏览器下载）：
          1. 环境变量：PLAYWRIGHT_CHROMIUM_EXECUTABLE / CHROME_BIN /
             CHROME_PATH / PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH
          2. PATH 中命令：chromium / google-chrome / chrome
          3. Windows 常见安装位置（Chrome / Edge）
          4. Linux 常见发行版与固定安装路径
          5. Playwright 浏览器缓存目录（base 镜像预置或手动 install）
             含 chrome-linux/chrome 与 chrome-headless-shell 两类，reverse 取最新版本
        """
        candidates = []

        # 1. 环境变量显式指定（最高优先级）
        for env_key in ("PLAYWRIGHT_CHROMIUM_EXECUTABLE", "CHROME_BIN",
                        "CHROME_PATH", "PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH"):
        ...
        # 5. Playwright 浏览器缓存目录（base 镜像预置 / 手动 install），reverse 取最新版
        glob_patterns = (
            os.path.expanduser("~/.cache/ms-playwright/chromium-*/chrome-linux/chrome"),
            "/home/node/.cache/ms-playwright/chromium-*/chrome-linux/chrome",
            "/ms-playwright/chromium-*/chrome-linux/chrome",
            "/home/node/.cache/ms-playwright/chromium_headless_shell-*/chrome-linux/headless_shell",
            "/home/node/.cache/ms-playwright/chromium_headless_shell-*/chrome-headless-shell-linux64/chrome-headless-shell",
            "/home/node/.cache/ms-playwright/chromium-*/chrome-headless-shell-linux64/chrome-headless-shell",
        )
    ...
    def snapshot_complex_components(self, page=None):
        ...
        # 技能根可能是只读挂载（/opt/openclaw-skills），中间截图目录必须落到可写区：
        # 有 SKILL_OUTPUT_DIR 时优先用其下 tmp/html_to_word，否则回退原相对路径。
        if not tmp_dir.is_absolute() and os.environ.get("SKILL_OUTPUT_DIR"):
            tmp_dir = Path(os.environ["SKILL_OUTPUT_DIR"]) / "tmp" / "html_to_word"
    ...
    def _update_fields_via_word(self, out_path):
        ...
        # Word COM 仅在 Windows 可用；Linux（容器内）无 MS Office / WPS，
        # 直接回退到 settings.xml 开关模式，避免尝试导入 pywin32 失败。
        if os.name != "nt":
            _log("非 Windows 环境，跳过 Word COM 更新域（回退 settings.xml 开关模式）", "WARNING")
            return False
