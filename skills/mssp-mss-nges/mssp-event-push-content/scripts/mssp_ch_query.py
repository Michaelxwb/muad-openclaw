...
行为:
    - 查询到 1 条或多条：只选择最新一条（seq 最大）的 GPTrawLog
    - 将最新一条保存到 --out 指定文件（默认 save_dir/gptrawlog_<criterion>.json）
    - 查询到多条时打印全部命中索引，便于确认选择依据
    - 查询到 0 条：打印"无数据"
...
def main():
    ...
    ap.add_argument("--out", help="最新一条 GPTrawLog 的保存路径")
    ap.add_argument("--save-dir", default=None, help="最新一条 GPTrawLog 默认保存目录")
    ap.add_argument("--no-save", action="store_true", help="不保存最新一条，仅打印摘要")
    args = ap.parse_args()
    ...
    try:
        ...
        # 无论命中一条还是多条，都只保存 seq 最大的最新一条 GPTRawLog。
        if not args.no_save:
            if args.out:
                save_path = args.out
            else:
