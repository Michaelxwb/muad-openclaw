...
# 多库支持：默认库 + dbs 列表（去重、保序）。可被 env HUNT_CH_DB 覆盖为单一库。
_TABLE = _CFG.get("table", "data_dr_ops_telemetry_log")
_default_db = _CFG.get("default_db")
_db_list = _CFG.get("dbs") or []
_ordered = [d for d in ([_default_db] + list(_db_list)) if d]
_DATABASES = []
for _d in _ordered:  # 去重保序
    if _d not in _DATABASES:
        _DATABASES.append(_d)
env_db = os.environ.get("HUNT_CH_DB")
if env_db:
    _DATABASES = [env_db]
if not _DATABASES:
    sys.stderr.write("[错误] 配置缺少 default_db / dbs\n")
    sys.exit(1)
DB = _DATABASES[0]
# 每个库的 库.表 全名（查询时对每个库分别下发，再聚合）
TABLES = [f"{d}.{_TABLE}" for d in _DATABASES]

# tenant 若未显式给，从事件基础信息的 tenantId 提示；本脚本 always 显式 --tenant
DEFAULT_TENANT = os.environ.get("HUNT_TENANT", "")
DEFAULT_DAYS = int(os.environ.get("HUNT_DAYS", "3"))
...
def find_fast_hits(cli, etype, value, tenant, days):
    """第1步-快速汇总：按库依次查询，**一旦某库命中即停止后续库查询**（同一 tenant 只会落在一个库）。
    只 SELECT 轻量聚合，命中即证明全网有扩散。返回 (总命中, 受影响主机数, 各库命中明细)。
    """
    cond = build_match_cond(etype, value)
    per_db = []
    for tbl in TABLES:
        q = (f"SELECT count() AS c, groupUniqArray(agentId) AS agents "
             f"FROM {tbl} "
        ...
        try:
            ...
            per_db.append((tbl, c))
            if c > 0:
                agent_set = {str(a) for a in (agents or []) if a}
                return c, len(agent_set), per_db  # 命中即短路
        except Exception as e:
            sys.stderr.write(f"[fast] {tbl} ERR: {str(e)[:200]}\n")
            per_db.append((tbl, None))
    return 0, 0, per_db


def breakdown_by_agent(cli, etype, value, tenant, days, limit=100):
    """按受影响主机/agent 聚合：每台主机的命中条数 + 涉及事件种类数 + 末次时间。
    **短路**：按库依次查，首个命中的库即为该 tenant 唯一的库，不再查其它库。
    返回 [(agentId, hits, evtKinds, lastSeen), ...] 按 hits 降序。
    """
    cond = build_match_cond(etype, value)
    for tbl in TABLES:
        ...
        try:
            rows = cli.execute(q)
            if rows:
                merged = [(str(a), int(h), int(k), l) for a, h, k, l in rows]
                merged.sort(key=lambda x: x[1], reverse=True)
                return merged[:limit]
        except Exception as e:
            sys.stderr.write(f"[byAgent] {tbl} ERR: {str(e)[:200]}\n")
    return []
...
def detail_events(cli, etype, value, tenant, days, agent_filter=None, evt_filter=None, limit=50):
    ...
    for tbl in TABLES:
        q = f"SELECT {sel} FROM {tbl} WHERE {where} ORDER BY insertTime DESC LIMIT {limit}"
        try:
            rows, ct = cli.execute(q, with_column_types=True)
            if rows:
                cols = [c[0] for c in ct]
                return [dict(zip(cols, r)) for r in rows]  # 命中即短路
        except Exception as e:
            sys.stderr.write(f"[detail] {tbl} ERR: {str(e)[:200]}\n")
    return []
...
def main():
    ...
    ap.add_argument("--company-id", dest="company_id", default=os.environ.get("HUNT_COMPANY_ID", ""),
                    help="客户数字 company_id（用于把命中 agentId 反查 MSSP 资产；建议提供）")
    ap.add_argument("--asset", action="store_true",
                    help="输出命中终端资产信息（需 --company-id）：把 agentId 反查资产库，每台主机只输出最全最新一条")
    ap.add_argument("--days", type=int, default=DEFAULT_DAYS, help="回溯时间窗(天)，默认3，可到30")
    ...
    print(f"查询库（{len(TABLES)} 个）: {', '.join(_DATABASES)}")
    cli = connect()

    # 汇总命中数恒先算，供后续 mode 决定是否继续（0 命中则告一段落）
    c = None; agents = None; per_db = []
    if args.mode in ("summary", "byagent", "detail", "all"):
        c, agents, per_db = find_fast_hits(cli, etype, args.value, args.tenant, args.days)

    if args.mode in ("summary", "all"):
        hline(f"[1/汇总] 全网命中统计（tenant={args.tenant}, {args.days}天, {len(TABLES)}库）")
        if c is None:
            print("查询失败(见stderr)")
        elif c == 0:
        ...
        if per_db:
            print("  分库命中：")
            for tbl, dc in per_db:
                print(f"    {tbl}: {'查询失败' if dc is None else f'{dc} 条'}")
    ...
    # 命中主机 → 终端资产信息（用 agentId 反查 MSSP 资产库，每台主机只输出最全最新的一条）
    if args.asset and c and args.company_id:
        hline(f"[4/命中终端资产] 全网排查命中终端（company_id={args.company_id}）")
        try:
            import asset_lookup
        except ImportError:
            sys.stderr.write("[资产] 无法加载 asset_lookup.py\n")
            asset_lookup = None
        if asset_lookup is not None:
            agent_rows = breakdown_by_agent(cli, etype, args.value, args.tenant, args.days, args.limit)
            if not agent_rows:
                print("（无命中主机）")
            for agent, hits, kinds, last in agent_rows:
                try:
                    res = asset_lookup.query_asset_for_agent(agent, args.company_id)
                except Exception as e:  # noqa: BLE001
                    print(f"\n⚠️ 全网排查命中终端（agentId={agent}，命中 {hits} 条）："
                          f"资产查询失败：{str(e)[:120]}")
                    continue
                sel = res.get("selected")
                if not sel:
                    print(f"\n⚠️ 全网排查命中终端（agentId={agent}，命中 {hits} 条）：未在资产库查询到资产信息")
                    continue
                print(f"\n⚠️ 全网排查命中 {asset_lookup.describe_short(sel)} 终端，命中 {hits} 条遥测日志"
                      f"（agentId={agent}，{res.get('select_reason')}）")
                print("  终端资产详情：")
                print(asset_lookup.describe_asset(sel))
