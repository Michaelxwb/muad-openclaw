...
def _build_headers(cookie: str) -> dict:
    return {
        ...
        "x-csrftoken": cookie_csrf(cookie),
        "x-requested-with": "XMLHttpRequest",
    }


def cookie_csrf(cookie: str) -> str:
    """提取 cookie 中的 csrf_token。"""
    if not cookie:
        return ""
...
def fetch_policy_results(cookie: str, company_id: str, dev_id_list: list,
    ...
    while True:
        ...
        response = request_with_retry("POST", POLICY_LIST_URL, headers, timeout=30, json=payload)
...
def generate_policy_message(
    ...
    # 落盘完整 JSON（备用，模板里不展示）→ SKILL_OUTPUT_DIR/reports/
...
def main():
    ...
    parser.add_argument("--dev-ids", type=str, default=None,
                        help="设备 ID 列表，JSON 数组如 '[338,222]'；不传则读 session（若有）")
