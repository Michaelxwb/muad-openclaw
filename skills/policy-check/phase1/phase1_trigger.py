...
流程：
  ...
  3. 下发策略检查任务
  4. 把 session（company_id/company_name/task_ids/dev_id_list）写入输出目录 last_session.json
...
muad 改造：登录态由 shared.get_cookie() 代管；session 写 SKILL_OUTPUT_DIR；
企微通知已移除，原 notify 改为 log。
"""
...
from shared import log, get_cookie, save_session as _save_session, clean_reports  # noqa: E402
...
def step1_get_company_id(cookie: str, company_name: str):
    ...
    try:
        ...
        log(f"[Step 1 失败] 客户确认失败: {e}", "ERROR")
        raise
...
def step2_get_dev_id_list(cookie: str, company_id: str, company_name: str):
    ...
    try:
        ...
        log(f"[Step 2 失败] 获取设备列表失败: {e}", "ERROR")
        raise
...
def step3_distribute_task(cookie: str, company_id: str, company_name: str, device_list: list):
    ...
    try:
        ...
        log(f"[Step 3 失败] 任务下发失败: {e}", "ERROR")
        raise
...
def save_session(company_id: str, company_name: str, task_ids: list, dev_id_list: list):
    """下发成功后写入输出目录 last_session.json，供 phase2/phase4 默认读取。"""
    _save_session({
    ...
    log("[Session] 已写入 last_session.json")
...
def run_phase1(cookie: str, company_name: str):
    ...
    log(f"[Phase 1] 策略检查任务启动，客户「{company_name}」")
    try:
        ...
        log(f"[Phase 1] 「{corrected_name}」获取到 {len(dev_id_list)} 台设备：{device_desc}")
        task_ids, type_count, skipped_count = step3_distribute_task(cookie, company_id, corrected_name, device_list)
        log(f"[Phase 1] 「{corrected_name}」策略检查任务已下发（{len(dev_id_list)}台设备，{len(task_ids)}个任务）")
        log("[Phase 1] 全部完成")
