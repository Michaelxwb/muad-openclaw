#!/usr/bin/env node
// Local probe for the mssp session adapter. Not part of the shipping CLI — use this
// to verify the signing algorithm against a live mssp endpoint during development.
// mssp 的登录逻辑与 mssw 完全一致，仅平台名与 X-Branch-Tag（MSSP-ADAPTER）不同。
//
// Usage:
//   node scripts/probe-mssp.mjs \
//     --ak <AK> --sk <SK> \
//     --url https://sitmssp.soar.sangfor.com/gateway/mss-auth-acl-service/v1/certification/login_agent \
...
//   MSYS_NO_PATHCONV=1 node scripts/probe-mssp.mjs --ak ... --health-endpoint /v1/rtt --validate
//
...
import { MSSPSessionAdapter } from "../dist/adapters/mssp.js";
...
async function main() {
  ...
  const adapter = new MSSPSessionAdapter(insecureSkipVerifyFetch);
  const credential = {
    humanUserId: "local-debug",
    podId: "local-debug",
    agentId: parsed.agentId,
    skillName: "mssp-query",
    platform: "mssp",
