...
export class SkillBusyError extends Error {
  constructor(message = "skill concurrency limit reached") {
    super(message);
...
export class SharedSkillLeaseManager {
  #queue;
  #pending = new Set();
  #releases = new Map();
  ...
  async acquire(key) {
    ...
    if (this.#pending.has(normalized) || this.#releases.has(normalized)) {
      throw new SkillBusyError("duplicate skill run");
    }
...
export class SkillLeaseManager {
  #active = new Map();
  #waiters = [];
  #watchdog;
  ...
  constructor(options) {
    ...
    if (![limit, waitTimeoutMs, leaseTtlMs, watchdogIntervalMs, maxQueue]
      ...
      throw new Error("invalid skill lease configuration");
    }
    this.limit = limit;
  ...
  close() {
    ...
    const error = new SkillBusyError("skill lease manager stopped");
  ...
  #grant(key, metadata) {
    ...
    waiter.timer = setTimeout(() => {
      ...
      waiter.reject(new SkillBusyError());
    }, this.waitTimeoutMs);
    waiter.timer.unref?.();
...
function normalizeKey(value) {
  ...
  if (!key) throw new Error("skill lease key is required");
  return key;
}
